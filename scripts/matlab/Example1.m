...
% giving some random numbers to the experimentally observed biomass weights in gdw
X_k_exp = rand(numel(modelNames),1)*1000;
X_k_exp = X_k_exp;
...
randIDS = sort(randsample(numel(exRxns),30));
exRxns = exRxns(randIDS);
exMets = findMetsFromRxns(model,exRxns);

% giving random numbers as consumption rate
rate = 1000*10*ones(numel(exRxns),1);

EX_.mets = exMets;
EX_.rxns = exRxns;
EX_.rate = rate;

[X_k, V] = comModel2(models, modelNames, X_k_exp, bio_id, X0, mu, EX_);

agg_flux=[];
for i=1:numel(exRxns)
    r = exRxns{i};
    ids = find(ismember(V.Reactions,'EX_glu_L(e)'));
    agg_flux = [agg_flux;sum(V.Aggregate_Flux(ids))];
end